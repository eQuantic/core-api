...
4. If a release is due, the pipeline — in order — updates `CHANGELOG.md`, stamps the version into
   `src/Directory.Build.props`, packs the thirteen packages with `-p:Version` +
...
`v1.9.3` is an annotated **baseline marker** placed on the pipeline commit: semantic-release
only counts from *stable* tags on a release branch, and 1.9.3 is the version published on
NuGet before automation. The commit `✨ feat!: eQuantic.Core.Api v2` right after the marker
declares the breaking modernization, so the first automated release is exactly **`2.0.0`**.
